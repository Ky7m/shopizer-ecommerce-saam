...
public sealed class IdentityRepository(NpgsqlDataSource dataSource)
{
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("DELETE FROM customer_identity.administrator_accounts WHERE id=@id AND tenant_id=@tenant AND store_id=@store", db);
        P(cmd, "id", id); P(cmd, "tenant", ctx.TenantId); P(cmd, "store", ctx.StoreId); await cmd.ExecuteNonQueryAsync(ct);
    }
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("SELECT id,reviewer_customer_id,reviewed_customer_id,rating,review_text,review_date,status::text FROM customer_identity.customer_reviews WHERE reviewed_customer_id=@target AND status <> 'Deleted' ORDER BY review_date", db);
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("SELECT id,reviewer_customer_id,reviewed_customer_id,rating,review_text,review_date,status::text FROM customer_identity.customer_reviews WHERE id=@id", db);
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("INSERT INTO customer_identity.customer_reviews(id,reviewer_customer_id,reviewed_customer_id,rating,review_text,review_date,status) VALUES(@id,@reviewer,@target,@rating,@text,@date,@status::customer_identity.review_status)", db);
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("UPDATE customer_identity.customer_reviews SET rating=@rating,review_text=@text,status=@status::customer_identity.review_status,updated_at=now() WHERE id=@id", db);
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("UPDATE customer_identity.customer_reviews SET status='Deleted',updated_at=now() WHERE id=@id", db); P(cmd, "id", id); await cmd.ExecuteNonQueryAsync(ct);
    }
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("UPDATE customer_identity.customer_accounts SET review_average=@average,review_count=@count,updated_at=now() WHERE id=@id", db); P(cmd, "average", average); P(cmd, "count", count); P(cmd, "id", target); await cmd.ExecuteNonQueryAsync(ct);
    }
    ...
    public async Task<ExternalIdentityRecord> AddExternalAsync(ExternalIdentityRecord e, CancellationToken ct)
    {
        ...
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("INSERT INTO customer_identity.external_identity_connections(user_id,provider_id,provider_user_id,access_token,refresh_token,profile_url) VALUES(@user,@provider,@providerUser,@access,@refresh,@profile)", db);
    ...
    {
        await using var db = await dataSource.OpenConnectionAsync(ct);
        await using var cmd = new NpgsqlCommand("UPDATE customer_identity.administrator_accounts SET is_active=@active,updated_at=now() WHERE id=@id AND tenant_id=@tenant AND store_id=@store", db); P(cmd, "active", enabled); P(cmd, "id", id); P(cmd, "tenant", ctx.TenantId); P(cmd, "store", ctx.StoreId); await cmd.ExecuteNonQueryAsync(ct);
    }
