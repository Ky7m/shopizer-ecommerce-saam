...
public sealed class CatalogRepository(NpgsqlDataSource dataSource, ILogger<CatalogRepository> logger)
{
        ...
        await using (var reader = await command.ExecuteReaderAsync(ct))
        {
            if (await reader.ReadAsync(ct))
            {
                product = new ProductRecord
                {
                    ...
                    RefSku = reader.IsDBNull(4) ? null : reader.GetString(4),
                    Status = reader.GetString(5),
                    Visible = reader.GetBoolean(6),
                    Available = reader.GetBoolean(7),
                    CanBePurchased = reader.GetBoolean(8),
                    DateAvailable = reader.GetFieldValue<DateTimeOffset>(9),
                    ManufacturerCode = reader.IsDBNull(10) ? null : reader.GetString(10),
                    ProductTypeCode = reader.IsDBNull(11) ? null : reader.GetString(11),
                    TaxClassCode = reader.IsDBNull(12) ? null : reader.GetString(12),
                    ProductVirtual = reader.GetBoolean(13),
                    ProductShippable = reader.GetBoolean(14),
                    ProductFree = reader.GetBoolean(15),
                    Length = reader.IsDBNull(16) ? null : reader.GetDecimal(16),
                    Width = reader.IsDBNull(17) ? null : reader.GetDecimal(17),
                    Height = reader.IsDBNull(18) ? null : reader.GetDecimal(18),
                    Weight = reader.IsDBNull(19) ? null : reader.GetDecimal(19),
                    ReviewAverage = reader.IsDBNull(20) ? 0 : reader.GetDecimal(20),
                    ReviewCount = reader.GetInt32(21),
    ...
    public async Task<(List<CategoryRecord> Items, int Total)> ListCategoriesAsync(RequestContext ctx, int page, int pageSize, string language, string? name, bool? visible, bool? featured, CancellationToken ct)
    {
        ...
        var result = new List<CategoryRecord>(); foreach (var id in categoryIds) { var item = await ReadCategoryAsync(c, null, id, ctx, language, ct); if (item is not null) result.Add(item); }
        return (result, total);
    }
