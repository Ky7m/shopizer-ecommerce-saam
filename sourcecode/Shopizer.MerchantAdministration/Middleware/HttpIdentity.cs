...
public static class HttpIdentity
{
    ...
    {
        if (http.User.Identity?.IsAuthenticated != true || !http.User.Kind().Equals(kind, StringComparison.OrdinalIgnoreCase)) throw new DomainException("UNAUTHORIZED", "Authentication token is invalid", 401);
        if (!http.User.HasRole(roles)) throw new DomainException("FORBIDDEN", "Administrator is not authorized for this operation", 403);
